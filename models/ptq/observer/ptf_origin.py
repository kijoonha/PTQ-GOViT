...
class PtfObserver(BaseObserver):
    ...
    def get_quantization_params(self, inputs, *args, **kwargs):
        ...
        # num_channel = inputs.shape[2]
        # if len(inputs.shape) ==4:  #이 코드는 전체 ptf적용할때 act_out때문에 추가한 코드임.
        ...
        #     unique_values = torch.unique(scale_mask)  # 텐서 내 고유 값들
        #     for v in unique_values:
        #         count_v = (scale_mask == v).sum().item()
        #         print(f"{v.item()}의 개수: {count_v}")
