...
from .layers_quant import (DropPath, HybridEmbed, Mlp, PatchEmbed, to_2tuple,
                           trunc_normal_)
from .ptq import QAct, QConv2d, QIntLayerNorm, QIntSoftmax, QLinear, QConv3d
...
def save_tensor_as_npy(tensor, file_name):

    save_dir = "/home/jhki/Video-Swin-Transformer/save_npy/sthv2/small/qact4"
    # 현재 시간 정보를 가져옴
    # current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # file_name_with_time = f"{file_name}_{current_time}.npy"
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.qact_attn1 = QAct(quant=quant,
                               ...
                               calibration_mode=cfg.CALIBRATION_MODE_A_attn,
                               observer_str=cfg.OBSERVER_A_attn,
                               quantizer_str=cfg.QUANTIZER_A)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, last_quantizer=None):
        # if self.qact2.quant:
        #     self.qact2.quantizer.observer.print_outlier_count()
        H, W = self.input_resolution
        B, L, C = x.shape
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.ape:
            ...
            self.qact1 = QAct(quant=quant,
                              ...
                              bit_type=cfg.BIT_TYPE_A,
                              calibration_mode=cfg.CALIBRATION_MODE_A_LN,
                              observer_str=cfg.CALIBRATION_MODE_A_LN,
                              quantizer_str=cfg.CALIBRATION_MODE_A_LN)
        else:
    ...
    def model_find_outlier(self):
        for m in self.modules():
            if type(m) is QAct:
                if m.observer_str =='outlier':
                    m.find_outlier = True
                    
    def model_find_outlier_close(self):
        for m in self.modules():
            if type(m) is QAct:
                if m.observer_str =='outlier':
                    m.find_outlier = False
...
def swin_tiny_patch4_window7_224(pretrained=False,
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            ...
            map_location='cpu',
            check_hash=True)
        results = model.load_state_dict(checkpoint['model'], strict=False)
...
def swin_small_patch4_window7_224(pretrained=False,
    ...
    """
    layernorm_class= QIntLayerNorm
    # if cfg.ivit_layernorm ==True:
    #     layernorm_class = IntLayerNorm_ivit
    model = SwinTransformer(patch_size=4,
                            window_size=7,
                            embed_dim=96,
                            depths=(2, 2, 18, 2),
                            num_heads=(3, 6, 12, 24),
                            norm_layer=layernorm_class,
                            quant=quant,
                            calibrate=calibrate,
                            input_quant=True,
                            cfg=cfg,
    ...
    # 모델에 state dict를 로드한다 (strict=False)
